...
The tree and linear learners optimize the empirical binary policy objective.
SVM and neural learners tune their surrogate fits by cross-validated policy
value.

## Bounded hinge SVM

For the default Gaussian kernel with hinge loss, `odrlITR` fits a
residual-weighted, penalized hinge SVM and then clips its decision score to
`[-1, 1]` with hard tanh:
